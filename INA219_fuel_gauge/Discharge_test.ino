/*
Discharge test of 1400mAh lithium-ion battery capacity with constant load of 60mA.
Plot the capacity% values over time from full battery to empty.

Log measurements of voltage,current from INA219 and state-of-charge(capacity%) from MAX17048
into microSD card. INA219 readings are converted to battery capacity% through lookup tables.
...
code for use by this app. If capacity reaches 0, the load is disconnected by P-chan MOSFET 
...
#define MAX_VC_DEVIATION 15
#define CHANGE_THRESHOLD 10
#define BATTERY_IR 0.4f      //internal resistance = (Voc - Vload)/load_current_A @50% batt
...
double mxvoltage = 0; // Variable to keep track of LiPo voltage
...
bool timer2Enabled=true;          //event switch

int cutoffCounter=0,deviationCounter=0;
int lastSavePercent=0;
int logTimer=0,elapsed=0;
...
void updateOLED(float voltage,int pcv,float totalConsumedMAH,int pcc,bool pwroff) {
  ...
  sprintf(line1,"%.2fV %d%%",voltage,pcv);
  line(0,0,line1);
  sprintf(line2,"%.2fv %d%%",mxvoltage,isoc);
  line(0,20,line2);
  sprintf(line3,"%.1fA %d%%",totalConsumedMAH,pcc);
  if (strlen(line3) > 10)
    sprintf(line3,"%.1f %d",totalConsumedMAH,pcc);
  line(0,40,line3);
  if (pwroff)
    line(50,40,"PWROFF");
}
...
void logData() {
  ...
  elapsed++;
  sprintf(line,"%d,%.2f,%d,%.2f,%d,%.1f,%d\r\n",elapsed,compensatedVoltage,pcv,mxvoltage,isoc,totalConsumedMAH,pcc);
...
void updateBatteryLevel(uint32_t period) {
  ...
  compensatedVoltage = voltage + (current_A * BATTERY_IR);
  ...
  logTimer++;
  if (logTimer%60 == 0) {     //log data every minute
    logData();
  }
}
...
void setup() {
  int pcc,pcv;
  //char columnHeader[]={"elapsed,inaVoltage,INA219,maxVoltage,soc,mAh,TotalmAh\r\n"};
  char columnHeader[]={"elapsed,inaVoltage,pcv,TotalmAh,pcc\r\n"};

  pinMode(CUTOFF_PIN,OUTPUT);
  digitalWrite(CUTOFF_PIN,HIGH);    //turn on P-chan MOSFET
  Serial.begin(115200);
  delay(2000);
  Serial.println("INA219 Fuel Gauge 1.2");
  ...
  // Check if prev file exists, if so, remove it
  if (SD.exists("/prev.csv")) {
    SD.remove("/prev.csv");
  }
  //save last data.csv file
  SD.rename("/data.csv", "/prev.csv");
  Serial.println(columnHeader);
  appendFile(SD,"/data.csv", columnHeader);
...
void loop() {
  ...
  if ((period  > period2) && timer2Enabled) {
    ...
    mxvoltage = lipo.getVoltage();
    soc = lipo.getSOC();
    isoc=static_cast<int>(std::round(soc));
    alert = lipo.getAlert();
    //if (alert) {
    //  Serial.println("MAX17048 alert triggered");
    //}
